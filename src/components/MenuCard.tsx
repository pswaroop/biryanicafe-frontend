...
//                 >
//                   {item.customize ? "Customize" : "Add to Cart"}
//                 </Button>
//               </motion.div>
//             </div>
//           </CardContent>
...
export function MenuCard({ item, index = 0 }: MenuCardProps) {
  ...
  const spiceIcons = Array.from({ length: item.spice_level }, (_, i) => i);

  const originalPrice = parseFloat(item.price);
  const hasDiscount = item.discount_percent > 0;
  const finalPrice = hasDiscount
    ? (originalPrice * (1 - item.discount_percent / 100)).toFixed(2)
    : item.price;
      ...
      >
        <Card className="overflow-hidden hover-elevate group h-full flex flex-col">
          <div className="relative aspect-[4/3] overflow-hidden">
            ...
            {hasDiscount && (
              <div className="absolute top-3 left-3 inline-flex items-center px-3 py-1 rounded-full text-sm font-semibold bg-red-600 text-white shadow-lg">
          ...
          <CardContent className="p-4 sm:p-5 flex-1 flex flex-col">
            <div className="flex-1">
              {/* --- UPDATED: Veg Badge moved here, aligned with the item name --- */}
              <div className="flex justify-between items-start gap-2 mb-2">
                <h3 className="font-serif font-bold text-lg sm:text-xl line-clamp-1">
                  {item.name}
                </h3>
                {item.is_vegetarian && (
